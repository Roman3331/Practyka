      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-4xl max-h-[85vh] flex flex-col">
          <Card blur="lg" className="flex flex-col border-white/10 shadow-2xl overflow-hidden min-h-0 h-full">
            <div className="p-4 sm:p-6 border-b border-white/5 flex justify-between items-center bg-white/5 shrink-0">
              <div>
                <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tighter leading-tight">
                  {initialData ? 'Редагувати тест' : 'Створити новий тест'}
                </h2>
                <p className="text-slate-500 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] mt-1">Налаштування параметрів та питань</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 sm:space-y-8 custom-scrollbar min-h-0">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-6">
                  {questions.map((q, qIndex) => (
                    <Card key={q.id} blur="sm" className="p-4 sm:p-6 border-white/5 bg-white/5 relative group/q">
                      <button type="button" onClick={() => removeQuestion(qIndex)} className="absolute top-2 right-2 sm:top-4 sm:right-4 p-2 text-slate-600 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all opacity-0 group-hover/q:opacity-100"><Trash2 size={16} className="sm:w-[18px] sm:h-[18px]" /></button>
                      <div className="space-y-4">
                        <div className="flex gap-3 sm:gap-4 items-start"><span className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-400 text-xs sm:text-base font-black shrink-0 border border-indigo-500/20">{qIndex + 1}</span><textarea value={q.text} onChange={e => updateQuestionText(qIndex, e.target.value)} placeholder="Введіть текст питання..." className="w-full bg-transparent border-none outline-none text-white text-base sm:text-lg font-bold resize-none py-1 placeholder:text-slate-700" rows={2} /></div>
                        <div className="grid gap-2 sm:gap-3 ml-0 sm:ml-12">
                          {q.options.map((o, oIndex) => (
                            <div key={o.id} className="flex items-center gap-2 sm:gap-3 group/opt">
                              <button type="button" onClick={() => setCorrectOption(qIndex, o.id)} className={`p-1.5 sm:p-2 rounded-lg transition-all shrink-0 ${q.correctOptionId === o.id ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-white/5 text-slate-700 hover:text-emerald-500 hover:bg-emerald-500/10 border border-white/5'}`}><CheckCircle2 size={16} className="sm:w-[18px] sm:h-[18px]" /></button>
                              <input value={o.text} onChange={e => updateOptionText(qIndex, oIndex, e.target.value)} placeholder={`Варіант ${oIndex + 1}`} className="flex-1 bg-white/5 border border-transparent focus:border-indigo-500/30 rounded-xl px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-slate-300 outline-none transition-all placeholder:text-slate-700" />
                              {q.options.length > 2 && (<button type="button" onClick={() => removeOption(qIndex, oIndex)} className="p-1.5 sm:p-2 text-slate-700 hover:text-red-500 transition-colors shrink-0"><X size={14} className="sm:w-[16px] sm:h-[16px]" /></button>)}
                            </div>
                          ))}
                          <button type="button" onClick={() => addOption(qIndex)} className="flex items-center gap-2 text-[9px] sm:text-[10px] font-black text-indigo-400 hover:text-white transition-colors uppercase tracking-widest w-fit mt-2 ml-1"><Plus size={10} className="sm:w-3 sm:h-3" /> Додати варіант</button>
                        </div>
                      </div>
                    </Card>
                  ))}
